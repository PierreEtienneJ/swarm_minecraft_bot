...
class StateBot(mybot.MyBot, threading.Thread):
    """
    StateBot herited from :py:class:`mybot.MyBot` and :py:class:`threading.Thread`.
    ...
    ----------
    master : str
        player name: master of the bot
    __current_state: str
        current state
    current_goal : mc.Vector3
        goal position
    current_objectif : mc.Entity
        objectif entity
    """
    def __init__(self, name: str, host: str, port: Union[str, int], player_name: str):
        """init bot and add self.on_msg on self.event_list with event 'chat'
    ...
    def state_machine(self):
        ...
        if self.current_state == "find_target":
            ...
            # [mc.TypeEntity.HOSTILE, mc.TypeEntity.ANIMAL, mc.TypeEntity.MOB])
